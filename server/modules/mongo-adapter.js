...
MongoDB.prototype.connect = function (cbk) {
    ...
    var url = 'mongodb://' + self.host + ':' + self.port;
    ...
    MongoClient.connect(url, {native_parser: true}, function (err, client) {
        ...
        if(err){
            console.log(new Date() + " | MongoDB Server Error in authentications.");
            console.log(err);
            self.dbconn = db;
            cbk(db);
        }
        else{
            console.log(new Date() + " | MongoDB Server Connection Established....");
            console.log("Current database", self.dbname);
